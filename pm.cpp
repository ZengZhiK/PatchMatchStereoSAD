        ...
        for (int x = cx - half; x <= cx + half; ++x) {
            for (int y = cy - half; y <= cy + half; ++y) {
                ...
                if (dsp < 0 || dsp > this->max_disparity) {
                    ...
                    // SAD
                    cost += abs(f1(y, x)[0] - mcolo[0]);
                    //std::cerr << f1(y, x) << ", " << mcolo << ", " << cost << std::endl;
                    //std::cerr << abs(f1(y, x)[0] - mcolo[0]) << ", " << cost << std::endl;
                }
            }
        }
...
#pragma omp parallel for
        for (int y = 0; y < rows; ++y)
            for (int x = 0; x < cols; ++x)
                costs[cpv](y, x) = plane_match_cost(planes[cpv](y, x), x, y, this->window_size, cpv);
    }
    ...
    void PatchMatch::spatial_propagation(int x, int y, int cpv, int iter) {
        ...
        for (auto it = offsets.begin(); it < offsets.end(); ++it) {
            ...
            float new_cost = plane_match_cost(p_neigb, x, y, this->window_size, cpv);
    ...
    void PatchMatch::view_propagation(int x, int y, int cpv) {
        ...
        float new_cost = plane_match_cost(new_plane, mx, my, this->window_size, 1 - cpv);
    ...
    void PatchMatch::plane_refinement(int x, int y, int cpv, float max_delta_z, float max_delta_n, float end_dz) {
        ...
        while (max_dz >= end_dz) {
            ...
            float new_cost = plane_match_cost(new_plane, x, y, this->window_size, cpv);
    ...
    void
    PatchMatch::median_filter(int cx, int cy, cv::Mat1f &disparity, const cv::Mat1b &valid, int ws, bool use_invalid) {
        int half = ws / 2;

        std::vector<float> disps_win;
        ...
        if (!disps_win.empty()) {
            ...
            if ((disps_win.size() % 2) != 0)
                disparity(cy, cx) = disps_win[disps_win.size() / 2];
            else
                disparity(cy, cx) = (disps_win[disps_win.size() / 2 - 1] + disps_win[disps_win.size() / 2]) / 2.0f;
        }
    }
    ...
    void PatchMatch::set(const cv::Mat3b &img1, const cv::Mat3b &img2) {
        ...
        //int wmat_sizes[] = {rows, cols, this->window_size, this->window_size};
        //this->weigs[0] = cv::Mat(4, wmat_sizes, CV_32F);
        //this->weigs[1] = cv::Mat(4, wmat_sizes, CV_32F);
        //precompute_pixels_weights(img1, this->weigs[0], this->window_size);
        //precompute_pixels_weights(img2, this->weigs[1], this->window_size);
...
#pragma omp parallel for
        ...
        //for(int y=0; y<rows; ++y)
        //{
        //weighted_median_filter(x, y, disps[0], weigs[0], lft_validity, this->window_size, false);
        //weighted_median_filter(x, y, disps[1], weigs[1], rgt_validity, this->window_size, false);
        //}
        //}

        // applying median filter to left and right view respectively
        for (int x = 0; x < cols; ++x) {
            for (int y = 0; y < rows; ++y) {
                median_filter(x, y, disps[0], lft_validity, 7, false);
                median_filter(x, y, disps[1], rgt_validity, 7, false);
            }
        }
    }
