...
namespace pm {
    ...
    private:
        ...
        void
        weighted_median_filter(int cx, int cy, cv::Mat1f &disparity, const cv::Mat &weights, const cv::Mat1b &valid,
                               int ws, bool use_invalid);

        void median_filter(int cx, int cy, cv::Mat1f &disparity, const cv::Mat1b &valid, int ws, bool use_invalid);
